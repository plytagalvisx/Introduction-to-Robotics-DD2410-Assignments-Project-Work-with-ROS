  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::controller_manager_msgs::HardwareInterfaceResources_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.resources.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<std::basic_string<char, std::char_traits<char>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<char>>>::stream(s, true ? std::string() : indent + "    ", v.resources[i]);
    }
    if (v.resources.empty() || true)
      s << "]";
  }
