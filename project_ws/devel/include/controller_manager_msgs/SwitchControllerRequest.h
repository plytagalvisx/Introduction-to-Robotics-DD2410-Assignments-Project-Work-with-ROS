...
struct Printer< ::controller_manager_msgs::SwitchControllerRequest_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::controller_manager_msgs::SwitchControllerRequest_<ContainerAllocator>& v)
  {
    if (false || !indent.empty())
      s << std::endl;
    s << indent << "start_controllers: ";
    if (v.start_controllers.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.start_controllers.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<std::basic_string<char, std::char_traits<char>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<char>>>::stream(s, true ? std::string() : indent + "    ", v.start_controllers[i]);
    }
    if (v.start_controllers.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "stop_controllers: ";
    if (v.stop_controllers.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.stop_controllers.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<std::basic_string<char, std::char_traits<char>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<char>>>::stream(s, true ? std::string() : indent + "    ", v.stop_controllers[i]);
    }
    if (v.stop_controllers.empty() || true)
      s << "]";
    if (true || !indent.empty())
