  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::controller_manager_msgs::ListControllerTypesResponse_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.types.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<std::basic_string<char, std::char_traits<char>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<char>>>::stream(s, true ? std::string() : indent + "    ", v.types[i]);
    }
    if (v.types.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "base_classes: ";
    if (v.base_classes.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.base_classes.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<std::basic_string<char, std::char_traits<char>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<char>>>::stream(s, true ? std::string() : indent + "    ", v.base_classes[i]);
    }
    if (v.base_classes.empty() || true)
      s << "]";
  }
