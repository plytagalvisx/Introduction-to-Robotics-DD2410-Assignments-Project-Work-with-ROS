  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::gazebo_msgs::ContactState_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.wrenches.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::geometry_msgs::Wrench_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.wrenches[i]);
    }
    if (v.wrenches.empty() || false)
      s << "]";
    if (true || !indent.empty())
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "contact_positions: ";
    if (v.contact_positions.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.contact_positions.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::geometry_msgs::Vector3_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.contact_positions[i]);
    }
    if (v.contact_positions.empty() || false)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "contact_normals: ";
    if (v.contact_normals.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.contact_normals.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::geometry_msgs::Vector3_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.contact_normals[i]);
    }
    if (v.contact_normals.empty() || false)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.depths.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.depths[i]);
    }
    if (v.depths.empty() || true)
      s << "]";
  }
