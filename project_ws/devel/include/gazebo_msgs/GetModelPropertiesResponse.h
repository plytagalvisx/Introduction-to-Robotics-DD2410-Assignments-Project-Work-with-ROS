  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::gazebo_msgs::GetModelPropertiesResponse_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.body_names.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<std::basic_string<char, std::char_traits<char>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<char>>>::stream(s, true ? std::string() : indent + "    ", v.body_names[i]);
    }
    if (v.body_names.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.geom_names.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<std::basic_string<char, std::char_traits<char>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<char>>>::stream(s, true ? std::string() : indent + "    ", v.geom_names[i]);
    }
    if (v.geom_names.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "child_model_names: ";
    if (v.child_model_names.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.child_model_names.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<std::basic_string<char, std::char_traits<char>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<char>>>::stream(s, true ? std::string() : indent + "    ", v.child_model_names[i]);
    }
    if (v.child_model_names.empty() || true)
      s << "]";
    if (true || !indent.empty())
