  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::pal_navigation_msgs::VisualLocDB_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.documents.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint8_t>::stream(s, true ? std::string() : indent + "    ", v.documents[i]);
    }
    if (v.documents.empty() || true)
      s << "]";
    ...
    for (size_t i = 0; i < v.tree.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint8_t>::stream(s, true ? std::string() : indent + "    ", v.tree[i]);
    }
    if (v.tree.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.weights.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint8_t>::stream(s, true ? std::string() : indent + "    ", v.weights[i]);
    }
    if (v.weights.empty() || true)
      s << "]";
  }
