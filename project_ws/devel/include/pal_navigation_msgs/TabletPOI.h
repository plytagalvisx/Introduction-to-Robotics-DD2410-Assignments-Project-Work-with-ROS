  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::pal_navigation_msgs::TabletPOI_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.POIs.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::geometry_msgs::Point_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.POIs[i]);
    }
    if (v.POIs.empty() || false)
      s << "]";
    ...
    for (size_t i = 0; i < v.IDs.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::std_msgs::String_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.IDs[i]);
    }
    if (v.IDs.empty() || false)
      s << "]";
    if (true || !indent.empty())
