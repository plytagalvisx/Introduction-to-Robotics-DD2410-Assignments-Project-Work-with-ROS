  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::pal_interaction_msgs::TtsMark_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.value.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<std::basic_string<char, std::char_traits<char>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<char>>>::stream(s, true ? std::string() : indent + "    ", v.value[i]);
    }
    if (v.value.empty() || true)
      s << "]";
  }
