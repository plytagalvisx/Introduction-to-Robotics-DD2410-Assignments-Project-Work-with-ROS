  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::pal_interaction_msgs::audiosignal_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.channel0.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.channel0[i]);
    }
    if (v.channel0.empty() || true)
      s << "]";
    ...
    for (size_t i = 0; i < v.channel1.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.channel1[i]);
    }
    if (v.channel1.empty() || true)
      s << "]";
  }
