  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::pal_vision_msgs::HogDetection_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.hog.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.hog[i]);
    }
    if (v.hog.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "rgbClusterCenters: ";
    if (v.rgbClusterCenters.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.rgbClusterCenters.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::std_msgs::ColorRGBA_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.rgbClusterCenters[i]);
    }
    if (v.rgbClusterCenters.empty() || false)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "rgbDescriptor1: ";
    if (v.rgbDescriptor1.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.rgbDescriptor1.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint32_t>::stream(s, true ? std::string() : indent + "    ", v.rgbDescriptor1[i]);
    }
    if (v.rgbDescriptor1.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "rgbDescriptor2: ";
    if (v.rgbDescriptor2.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.rgbDescriptor2.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint32_t>::stream(s, true ? std::string() : indent + "    ", v.rgbDescriptor2[i]);
    }
    if (v.rgbDescriptor2.empty() || true)
      s << "]";
  }
