...
struct Printer< ::pal_web_msgs::GetUserInputGoal_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::pal_web_msgs::GetUserInputGoal_<ContainerAllocator>& v)
  {
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "valid_asr_inputs: ";
    if (v.valid_asr_inputs.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.valid_asr_inputs.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<std::basic_string<char, std::char_traits<char>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<char>>>::stream(s, true ? std::string() : indent + "    ", v.valid_asr_inputs[i]);
    }
    if (v.valid_asr_inputs.empty() || true)
      s << "]";
    if (true || !indent.empty())
