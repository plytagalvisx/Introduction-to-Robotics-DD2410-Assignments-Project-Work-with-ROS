  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::pal_detection_msgs::RecognizedActions_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.action_name.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<std::basic_string<char, std::char_traits<char>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<char>>>::stream(s, true ? std::string() : indent + "    ", v.action_name[i]);
    }
    if (v.action_name.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.u.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int64_t>::stream(s, true ? std::string() : indent + "    ", v.u[i]);
    }
    if (v.u.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.v.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int64_t>::stream(s, true ? std::string() : indent + "    ", v.v[i]);
    }
    if (v.v.empty() || true)
      s << "]";
  }
