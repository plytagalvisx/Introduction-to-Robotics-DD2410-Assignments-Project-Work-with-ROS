...
struct Printer< ::pal_control_msgs::ActuatorCurrentLimit_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::pal_control_msgs::ActuatorCurrentLimit_<ContainerAllocator>& v)
  {
    if (false || !indent.empty())
      s << std::endl;
    s << indent << "actuator_names: ";
    if (v.actuator_names.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.actuator_names.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<std::basic_string<char, std::char_traits<char>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<char>>>::stream(s, true ? std::string() : indent + "    ", v.actuator_names[i]);
    }
    if (v.actuator_names.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "current_limits: ";
    if (v.current_limits.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.current_limits.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.current_limits[i]);
    }
    if (v.current_limits.empty() || true)
      s << "]";
  }
