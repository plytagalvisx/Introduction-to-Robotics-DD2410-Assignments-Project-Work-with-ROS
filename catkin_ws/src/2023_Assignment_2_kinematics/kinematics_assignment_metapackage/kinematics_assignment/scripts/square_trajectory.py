...
class SquareTrajectory():
    ...
    def next_segment(self):
        ...
        # My own extension:
        if self._current_segment >= len(self._vertices) - 1:
    ...
    # Make our trajectory accept ANY number of vertices
    # Generic version: now the class can follow any number of vertices
    def compute_waypoints(self):
        ds = self._v * self._dt
        self._waypoints = []

        for i in range(len(self._vertices)):
            p1 = np.array(self._vertices[i])
            p2 = np.array(self._vertices[(i + 1) % len(self._vertices)])  # wrap-around to close
            ...
            if abs(float(it)) < 1e-6:
                ...
                dv = v / float(it)
                seg = self.return_list_of_waypoints(p1, p2, dv, it)

            self._waypoints.append(seg)
